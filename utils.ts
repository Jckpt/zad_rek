...
export function getHighestValueDrop(data: Item[]) {
    ...
    return {priceDrop: highestValueDrop.toFixed(2), index: highestValueDropIndex};
}
...
export function getValueDropPeriods(data: Item[]){
    ...
    data.forEach((currentItem,i)=>{
        ...
        if (didValueDrop && (lastPrice <= currentItem.price || i === data.length - 1)) {
            didValueDrop = false;
            lastPrice = currentItem.price;
            valueDropPeriods++;
            return;
...
export function getHighestValueDropSum(data: Item[]){
    ...
    return {highestValueDropSum, daysCount: highestValueDropSumDaysCount, valueDropStart: highestValueDropSumStart};
}
...
export function getHighestConsecutiveValue(data: Item[]){
    ...
    data.forEach((currentItem,i)=>{
        ...
        if (didValueStay && (lastPrice !== currentItem.price || i === data.length - 1)) {
    ...
    return {highestConsecutiveValue, daysCount: highestDaysCount, consecutiveValueStart: highestConsecutiveValueStart};
}